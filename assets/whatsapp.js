...
window.WAPI.getUnreadMessages2 = function (done) {
    ...
    chats.forEach(chats_man => {
        if(chats_man.unreadCount > 0){
            var is_group = chats_man.__x_isGroup;
            var chat_title = chats_man.__x_formattedTitle;

            var all_loaded_chats = chats_man.msgs._models;
            msg_to_un = all_loaded_chats.slice(Math.max(all_loaded_chats.length - chats_man.unreadCount, 1));
            msg_to_un.reverse();

            msg_arr = [];
            msg_to_un.forEach(msg_to => {
                var conster;
                msg_to.__x_author != undefined ? conster = msg_to.__x_author.user : conster = msg_to.__x_from.user;
                var onmg = {
                    body: msg_to.__x_body,
                    contact: conster,
                    from: msg_to.__x_from._serialized
                }
                msg_arr.push(onmg);
            });
            
            objd = {
                formattedName: chat_title,
                isGroup: is_group,
                messages: msg_arr
            };
            arp.push(objd);
